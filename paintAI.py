        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                if len(points) != 0:
                    ...
                    # After calculating 'fingers' and before other gesture logic, add:
                    if all(fingers[i] == 0 for i in range(0, 5)):
                        imgCanvas = np.zeros((height, width, 3), np.uint8)
                        xp, yp = [x1, y1]

                    # --- Erase all gesture: open then close hand (tolerant, with grace period) ---
                    open_palm = (fingers[1:] == [1, 1, 1, 1])  # Ignore thumb
                    closed_fist = (fingers[1:] == [0, 0, 0, 0])
                    if open_palm:
                        was_hand_open = True
                        hand_open_time = time.time()
                    elif closed_fist and was_hand_open:
                        # Allow a 1-second grace period after open palm
                        if 'hand_open_time' in locals() and (time.time() - hand_open_time) <= 1.0:
                            imgCanvas = np.zeros((height, width, 3), np.uint8)
                            xp, yp = [0, 0]
                            was_hand_open = False
